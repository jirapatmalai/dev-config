...
# Flutter
if [ ! -n "${BULLETTRAIN_FLUTTER_BG+1}" ]; then
  BULLETTRAIN_FLUTTER_BG=cyan
fi
if [ ! -n "${BULLETTRAIN_FLUTTER_FG+1}" ]; then
  BULLETTRAIN_FLUTTER_FG=black
fi
if [ ! -n "${BULLETTRAIN_FLUTTER_PREFIX+1}" ]; then
  BULLETTRAIN_FLUTTER_PREFIX="📱 flutter"
fi
...
prompt_flutter() {
  ...
  if [[ -f pubspec.yaml ]]; then
    if command -v flutter > /dev/null 2>&1; then
      prompt_segment $BULLETTRAIN_FLUTTER_BG $BULLETTRAIN_FLUTTER_FG $BULLETTRAIN_FLUTTER_PREFIX
      # local flutter_version
      # flutter_version=$(flutter --version | head -n 1 | awk '{print $2}')
      # prompt_segment $BULLETTRAIN_FLUTTER_BG $BULLETTRAIN_FLUTTER_FG $BULLETTRAIN_FLUTTER_PREFIX" $flutter_version"
    fi
  fi
}
